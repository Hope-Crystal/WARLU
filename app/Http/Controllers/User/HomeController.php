...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //transactions
        $transactions = $this->user->transaction()->orderBy('id', 'DESC')->limit(4)->where('user_id', $this->user->id)->paginate(config('basic.paginate'));
        ...
        $data['escrow'] = collect(Escrow::where(function ($query) use ($auth) {
            ...
            return view($this->theme . 'user.dashboard', $data,  compact('transactions', 'funds', 'user', 'languages'));
    ...
    public function transaction()
    {
        ...
        $user = $this->user;
        $languages = Language::all();
        return view($this->theme . 'user.transaction.index', compact('transactions', 'user'));
    ...
    public function fundHistory()
    {
        ...
        $user = $this->user;
        $languages = Language::all();
        return view($this->theme . 'user.transaction.fundHistory', compact('funds', 'user'));
    ...
    public function addFund()
    {
        ...
        return view($this->theme . 'user.addFund', $data, compact('user'));
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        return back()->with('success', 'Updated Successfully.','user');
    }
    ...
    public function twoStepSecurity()
    {
        ...
        return view($this->theme . 'user.twoFA.index', compact('secret', 'qrCodeUrl', 'previousCode', 'previousQR', 'user'));
    }
    ...
    public function payoutMoney()
    {
        ...
        return view($this->theme . 'user.payout.money', $data, compact( 'user' ));
    }
    ...
    public function payoutPreview()
    {
        ...
        $title = "Payout Form";
        return view($this->theme . 'user.payout.preview', compact('withdraw', 'title', 'user'));
    }
    ...
    public function payoutHistory()
    {
        ...
        return view($this->theme . 'user.payout.log', $data,  compact('user', 'languages'));
    }
    ...
    public function payoutHistorySearch(Request $request)
    {
        ...
        $title = "Payout Log";
        return view($this->theme . 'user.payout.log', compact('title', 'payoutLog', 'user'));
    ...
    public function makeEscrow()
    {
        ...
        return view($this->theme . 'user.escrow.create', compact('contactList', 'user'));
    }
...
//        'invitor','invitee','both'
        ...
        session()->flash('success', 'Your request has been sent, Click on my Escrow to view the status of your request.');
    ...
    public function myContactList()
    {
        ...
        return view($this->theme . 'user.escrow.contact-list', compact('contactList', 'user'));
    }
    ...
    public function checkContact(Request $request)
    {
        ...
        $user = User::where('id', '!=', $auth->id)->where(function ($query) use ($search) {
            return $query->where('email', "{$search['search']}")
                ->orWhere('username', "{$search['search']}")
                ->orWhere('phone', "{$search['search']}");
        })
            ->where('status', 1)
            ->first();

        if (!$user) {
            return response(['search' => ["``" . $search['search'] . "`` does not exist in our records!"]], 422);
        }
        ...
        if ($contact) {
            if ($contact->status == 2) {
                return response(['search' => ["You're not eligible to connected with ``" . $search['search'] . "``."]], 422);
            }
            if ($contact->status == 1) {
                return response(['search' => ["Already you're connected with ``" . $search['search'] . "``."]], 422);
            }
            if ($contact->status == 0 && $contact->user_id == $auth->id) {
                return response(['search' => ["Already sent a request connection to ``" . $search['search'] . "``."]], 422);
            }
            if ($contact->status == 0 && $contact->receiver_id == $auth->id) {
                return response(['search' => ["``" . $search['search'] . "`` sent a request connection to you,  Please accept his/her request."]], 422);
            }
        }
        ...
        return response(['success' => 'Invitation has been sent to ' . " ``" . $search['search'] . "``"]);
    ...
    public function myEscrow()
    {
        ...
        return view($this->theme . 'user.escrow.index', compact('myEscrow', 'user'));
    }
    ...
    public function reports($id)
    {
        ...
        return view($this->theme . 'user.escrow.chat', compact('escrow', 'auth','data', 'user'));
