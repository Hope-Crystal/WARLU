...
@section('content')
    ...
    <div id="wrapper">
    <section class="account-section overflow-hidden content-wrapper">
        <div class="container">
                ...
                <div class="card__body">
                    ...
                    <form role="form" id="submitForm" method="post" action="{{route('user.saveEscrow')}}" enctype="multipart/form-data">
                        ...
                        <div class="step__wrapper setup-content" id="step-1">
                            <h5 class="title text-center">@lang('Product / Service Name')</h5>
                            <div class="mb-3 form-group">
                                <input type="text" name="title" value="{{old('title')}}" class="form-control form--control"
                                       placeholder="{{trans('Product name')}}" required="required">
                            </div>
                        ...
                        <div class="step__wrapper setup-content" id="step-2">
                            <h5 class="title text-center">@lang('Package Descriptions')</h5>
                            <div class="mb-3 form-group">
		                        <textarea name="rules" id="rules" rows="5"
                                          class="form-control form--control rounded p-4"
                                          required="required"
                                          placeholder="@lang('Enter the description of the package')">{{old('rules')}}</textarea>
                            </div>
                        ...
                        <div class="step__wrapper setup-content" id="step-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <h5 class="title text-center">@lang('Invite a Transaction Partner')</h5>
                                        ...
                                        <a href="javascript:void(0)" class="stake__opponent__item"
                                           data-id="{{$item['id']}}">
                                            <img src="{{getFile(config('location.user.path').@$item['info']['image'])}}"
                                                ...
                                                <span style="font-size: 12px; padding:10px; ">@lang(@$item['info']['profileName'])</span>
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="step__wrapper setup-content" id="step-4">
                            <h5 class="title text-center">@lang('Value of the transaction?')</h5>
                            <div class="form-group mb-3">
                                <div class="input-group">
                            ...
                            <div class="form-group mb-3">
                                <label >{{trans('Who will bear the Escrow charge')}} <code>({{getAmount($basic->escrow_charge)}}{{trans('% Charge Apply')}})</code></label>
                                ...
                                <div class="d-flex form-check-input-font">
                                    ...
                                    <div class="form-check form-check-inline">
                                        ...
                                        <label class="form-check-label" for="charge_bear2">{{trans('Other Party')}}</label>
                                    </div>
                                    <div class="form-check form-check-inline">
...
@push('script')
    <script>
        ...
        $(document).on('click', '.submit', function () {
            ...
            if (title.length == 0) {
                Notiflix.Notify.Info(`{{trans('Please enter product title.')}}`);
                return 0;
            }
            if (rules.length == 0) {
                Notiflix.Notify.Info(`{{trans('Please enter the description of the package')}}`);
                return 0;
            }
            ...
            if (opponent == null) {
                Notiflix.Notify.Info(`{{trans('Please select your Transaction Partner for invite.')}}`);
                return 0;
            }
            $("#submitForm").submit();

            // $('.added').on('click', function () {
            //         Notiflix.Notify.Success(`{{trans('Successfully added, You may proceed to engage Escrows.')}}`);
            // });
            // window.location = '{{ url("escrow/My Escrow")}}';
